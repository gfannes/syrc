&&:syrc &#19 &2025

# Client
- [x] Measure/print duration
- [x] Rework comm to use `std.Io.Reader/Writer` iso generic
	- Add `flush()` where necessary
- Copy data from netstream directly to file iso buffer
- [x] Fix running in fast mode wrt Error.ExpectedChecksum
	- Issue was incorrect setup of `?[32]u8`
- [x] Support running rake commands in non-git copy of root-all
- [x] Fix failing `rake ut` due to missing `core/vlc/extern/lvgl/conf/lv_conf.h` in copied folder
	- Reworked existing .gitignore files
- [x] Export CSV with collected file info to reduce loaded file sizes
- [x] Print output in client
- [x] Emit stdout/stderr on actual stdout/stderr
	- [x] Extra logging only in verbose=2
- [x] Use default mode client
- [x] Indicate stdout/stderr in Session.processOutput_()
- [x] Fix `rake ut[core/audio]` over syrc
	- Due to missing `sine440.wav`
- [x] Rework protocol to support early sending of items
	- Main use case is a large tree with just a few updates
		- Send FileState during discovery
		- Do not send Missing/Content while still receiving FileStates
- [x] Implement Collect
- [x] Support for reusing an existing tree without removal
- Reuse object files by moving the cache outside of core/build/cache/cook
- [/] Create tree state
	- [x] List all files
		- [x] Basic walking of CWD in [[app.zig]]
	- [x] Add attribute info
	- [ ] Add additional files/folders with `.syrc` files, next to `.ignore` and `.gitignore` that overrule these
	- [x] Add checksum info
		- Blake3 at 128bit is secure enough, faster and shorter
		- [ ] Use a thread pool directly to queue the checksum tasks
			- [ ] Limit the queue size with a semaphore, by default to the CPU count
		- Read files into buffers
			- Is this faster MT?
		- Compute checksum
			- Is this faster MT?
- [x] Support cleanup blob.Store
- [x] Integrate Store into runTest()
	- [x] Create in rubr.fs.homeDir()/.config/syrc
	- [x] Use [32]u8 iso [64]u8 as Key in Store: couple with crypto.Checksum
	- [x] Implement basic API: check for content, add and extract
	- [x] Compute missing content when receiving a Replicate command
	- [x] Support bidirectional comms in runTest()
		- Use localhost stream

- [x] Don't crash the server when the command is not known
- [x] Transmit/set executable flag
- [x] Fix windows readonly flag handling in zig std itself in `lib/std/Io/File.zig`.`Permissions`
```
    pub fn readOnly(self: @This()) bool {
        return toAttributes(self).READONLY;
    }
	
```

- [x] Only update file when different: content or attr
- [x] Use default verbose 1: print basic info

- [/] Support cancelling an job
	- See `snippet/signal.zig` for a working example
	- [x] Integrate basic signal trap
	- [ ] Support stopping collecting file tree
		- [?] Distribute data via rubr.Env?
	- [ ] Support stopping sending blobs
- [x] Support setting/clearing defines at remote site
	- [x] Only use `-D` to set/clear a define
		- `-D abc`: clear
		- `-D abc=`: set to ``
		- `-D abc=def`: set to `def`
- [x] Overhaul CLI
	- [x] Interpret `abc=def` automatically as a define before `--`
	- [x] Only interpret alias as IP if it is actually found. Currently, `syrc rake` tries to use `rake` as ip, resulting in an `error.ParseFailed`.
- [ ] Support multiple connections from server
	- [x] Add a hostname to the protocol
	- [x] Support overriding this hostname via config
	- [x] Support an additional suffix via CLI
	- [x] Use hostname to create different folders, maybe with a session id as well
	- [*] Fail if the required folder is currently in-use
	- [ ] Handle each request in a thread
		- [ ] Split Server.processOne() into an accept and push actual processing into a thread
- [/] Support for creating aliases in `~/.config/syrc/aliases.zon`
	- Support for interpreting `name` before `--`
	- [*] `syrc` without arguments should print these
	- [*] Support adding/removing these with `-a` option

- [ ] Reduce 10GB data that is loaded for root-all
	- Remove `cli/espcap_ahp/qc/scenario_espcap_v2/auro-440/bianca/FhG_src_drop_renderers_v1.0.0_20180828_0bfbf8ce.zip`
	- Many `.wav` files

# Modules
- crypto
	- Secret
		- Sedes from file
	- Sign with HMAC-SHA256: HMAC(Secret, Time+Message)
	- Hash
- cli
	- Args
- cfg
	- Config
		- name, server, port
- mdl
	- Tree
- store
	- Object/File
	- Copy file parts efficiently: https://cfengine.com/blog/2024/efficient-data-copying-on-modern-linux/
- brkr
	- Broker
- clnt
	- Client
- srvr
	- Server
- net
- msg
	- Message
		- version, id
		- read(), write()
		- Type(u16), Size(u48), Data(zon, string)
- rubr
	- Support for collecting func from rubr automatically in a single file
- app
- main
	- `supr -C ./ -s abc cmd args`
